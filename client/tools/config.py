...
def init_config() -> dict[str, str | int]:
    ...
    try:
        ...
        config_params["client_handler_address"] = (
            os.getenv("CLIENT_HANDLER_ADDRESS")
            or config["DEFAULT"]["CLIENT_HANDLER_ADDRESS"]
        )
        config_params["dataset_path"] = (
            os.getenv("DATASET_PATH") or config["DEFAULT"]["DATASET_PATH"]
        )
        config_params["results_path"] = (
            os.getenv("RESULTS_PATH") or config["DEFAULT"]["RESULTS_PATH"]
        )
        config_params["max_retries"] = int(
            os.getenv("MAX_RETRIES") or config["DEFAULT"]["MAX_RETRIES"]
        )
        config_params["retry_delay"] = int(
            os.getenv("RETRY_DELAY") or config["DEFAULT"]["RETRY_DELAY"]
        )
        config_params["backoff_factor"] = int(
            os.getenv("BACKOFF_FACTOR") or config["DEFAULT"]["BACKOFF_FACTOR"]
        )
