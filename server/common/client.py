...
class Client:
    ...
    def send_message(self, data_send):
        if type(self.batch_processor) == MoviesProcessor:
            queue_to_send = self.queue_to_send(len(self.worker.output_exchange1.routing_keys))
            ...
            self.send_to_joiner_worker(data_send, self.worker.output_exchange2.routing_keys, self.worker.output_exchange2)
