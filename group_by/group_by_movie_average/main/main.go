...
func main() {
	...
	outputRoutingKeysMovieAvg := strings.Split(v.GetString("ROUTINGKEYS_OUTPUT_MASTER-GROUP-BY-MOVIE-AVERAGE"), ",")

	filterRoutingKeysMap := map[string][]string{
		"movie_avg": outputRoutingKeysMovieAvg,
	}
	...
	go func() {
		defer wg.Done()
		common_statefull_worker.RunWorker(group_by, ctx, group_by.Worker, "Starting group by movie average worker")
		done <- true
	}()
