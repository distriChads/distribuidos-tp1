...
type JoinMovieRatingById struct {
	...
	pending_ratings     map[string]map[string]string
}
...
func NewJoinMovieRatingById(config JoinMovieRatingByIdConfig, storage_base_dir string) *JoinMovieRatingById {
	...
	pending_ratings, _, _ := common_statefull_worker.GetPending[string](storage_base_dir)
	...
	for {
		...
		if inputIndex == 0 { // recibiendo movies
			...
			if !f.received_movies {
				...
				if _, ok := f.pending_ratings[client_id]; !ok {
					f.pending_ratings[client_id] = make(map[string]string)
				}
				pendings_for_client := f.pending_ratings[client_id]
				pendings_for_client[message_id] = message_str
				common_statefull_worker.StorePending(f.pending_ratings[client_id], client_id, f.storage_base_dir)
				msg.Ack(false)
				continue
			}

			if len(f.pending_ratings[client_id]) != 0 {
				...
				delete(f.pending_ratings, client_id)
			}
